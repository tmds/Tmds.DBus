...
// Extension methods providing RequestNameAsync/ReleaseNameAsync for tests.
// These APIs are not part of the Connection public API in this version.
static class ConnectionNameExtensions
{
    [Flags]
    ...
    public static Task RequestNameAsync(this Connection connection, string name, RequestNameOptions options = RequestNameOptions.None)
    {
        ...
        return connection.CallMethodAsync(message);
    }

    public static async Task<bool> ReleaseNameAsync(this Connection connection, string name)
    {
        MessageBuffer message;
        using (var writer = connection.GetMessageWriter())
        {
        ...
        uint reply = await connection.CallMethodAsync<uint>(message,
            (Message m, object? s) => m.GetBodyReader().ReadUInt32()).ConfigureAwait(false);
        return reply == 1; // 1 = Released
    }
}
...
public class SignalOwnerTests
{
    ...
    public async Task AddMatchFollowsNameOwner()
    {
        ...
        // Service provider
        using var serviceConnection = new Connection(dbusDaemon.Address!);
        await serviceConnection.ConnectAsync();
        await serviceConnection.RequestNameAsync(wellKnownName, ConnectionNameExtensions.RequestNameOptions.AllowReplacement);

        // Client connection
        using var clientConnection = new Connection(dbusDaemon.Address!);
        ...
        using var evilConnection = new Connection(dbusDaemon.Address!);
        ...
        using var service2 = new Connection(dbusDaemon.Address!);
        await service2.ConnectAsync();
        await service2.RequestNameAsync(wellKnownName, ConnectionNameExtensions.RequestNameOptions.ReplaceExisting);
        ...
        static void SendSignal(Connection conn, string message)
        {
    ...
    public async Task AddMatchForDBusDoesNotWatchOwner()
    {
        ...
        using var client = new Connection(dbusDaemon.Address!);
        ...
        using var otherConn = new Connection(dbusDaemon.Address!);
        ...
        using var anotherConn = new Connection(dbusDaemon.Address!);
    ...
    public async Task AddMatchWhenNoOwner()
    {
        ...
        static void SendSignal(Connection conn, string message = "signal")
        {
            using var writer = conn.GetMessageWriter();
            writer.WriteSignalHeader(
                path: "/com/example/Object",
