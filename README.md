...
When a client connects to the daemon, it is assigned a *local name*. In case it is a service provider, it will register specific *service names*. A service consumer can query the bus for the registered services. Certain services can also be started by the daemon (*activatable* services). This relies on configuration files which tell the daemon what application will provide a specific service.
